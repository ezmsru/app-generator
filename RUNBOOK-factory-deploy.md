...
## 6. Версионирование (теги)

- Тег обязателен — **без свободного нового тега деплоя не будет** ни на один стенд.
- Формат — semver: `0.0.x`, `0.y.x`, `x.y.z` (числа).
- **Пушить тег вместе с коммитом:** `git push --tags` (или галка *Push tags* в IDE) — по умолчанию теги в origin не уходят.
- Версия определяется по последнему тегу в source-ветке: `git describe --abbrev=0 --tags`.
